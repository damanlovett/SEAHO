...
if($_SESSION['accesspoint']!="CMS") {
		session_destroy();
		header ("Location: /registration/associates/index.php");
		exit;
}
...
if($_SESSION['systemStatus'] == "Inactive") {
		header ("Location: /registration/associates/index.php?error=11");
		exit;
}


// Assign User Information
if ((isset($_POST['associateemail'])) && (isset($_POST['password']))) {
$colname_rsUserInfo = "-1";
if (isset($_POST['associateemail'])) {
  $colname_rsUserInfo = $_POST['associateemail'];
}
...
mysql_select_db($database_CMS, $CMS);
$query_rsUserInfo = sprintf("SELECT * FROM associate WHERE email = %s AND associate.password = %s AND 'delete' = 0", GetSQLValueString($colname_rsUserInfo, "text"),GetSQLValueString($colname2_rsUserInfo, "text"));
...
if($totalRows_rsUserInfo == '0') {
		header ("Location: /registration/associates/index.php?error=4");
		exit;
}

//User is not active
if($row_rsUserInfo['active'] != '1') {
		header ("Location: /registration/associates/index.php?error=3");
		exit;
}
...
$_SESSION['userID'] = $row_rsUserInfo['associate_id'];
...
if ((isset($_GET['doLogout'])) &&($_GET['doLogout']=="true")){
  ...
  $logoutGoTo = "/registration/associates/index.php?error=1";
  if ($logoutGoTo) {
