...
mysql_select_db($database_Programming, $Programming);
$query_rsPrograms = sprintf("SELECT callforprograms.id, callforprograms.ProgramTitle, callforprograms.ProgramNumber, callforprograms.`session`, callforprograms.location, callforprograms.SessionType, callforprograms.FirstName, callforprograms.LastName, callforprograms.EmailAddress, callforprograms.TopicArea, callforprograms.Status FROM callforprograms WHERE `session` = %s OR ProgramTitle LIKE CONCAT('%%', %s, '%%') OR FirstName LIKE CONCAT('%%', %s, '%%') OR LastName LIKE CONCAT('%%', %s, '%%') OR TopicArea = %s ".$_POST['sortby']."", 
...
mysql_select_db($database_Programming, $Programming);
$query_rsPrograms = sprintf("SELECT callforprograms.id, callforprograms.ProgramTitle, callforprograms.ProgramNumber, callforprograms.`session`, callforprograms.location, callforprograms.SessionType, callforprograms.FirstName, callforprograms.LastName, callforprograms.EmailAddress, callforprograms.TopicArea, callforprograms.Status, COUNT(reviewers.programID) AS num_reviewers, reviewers.programID, reviewers.reviewID FROM callforprograms LEFT JOIN reviewers ON callforprograms.id = reviewers.programID GROUP by callforprograms.id ORDER BY callforprograms.submission_date DESC");
$query_limit_rsPrograms = sprintf("%s LIMIT %d, %d", $query_rsPrograms, $startRow_rsPrograms, $maxRows_rsPrograms);
$rsPrograms = mysql_query($query_limit_rsPrograms, $Programming) or die(mysql_error());
$row_rsPrograms = mysql_fetch_assoc($rsPrograms);
}
...
?>
	...
	<?php if ($totalRows_rsPrograms > 0) { // Show if recordset not empty ?>
      <table border="0" cellpadding="5" cellspacing="0" class="tableborder">
        <tr>
          <td colspan="7" class="tableTop"><input name="Submit2" type="button" onclick="MM_goToURL('parent','<?php echo $_SERVER['PHP_SELF'];?>');return document.MM_returnValue" value="Refresh List" /><?php if ($pageNum_rsPrograms > 0) { // Show if not first page ?>
...
?> class="tableRowColor" >
            ...
            <td nowrap="nowrap"><a href="editProgram.php?recordID=<?php echo $row_rsPrograms['id']; ?>"><img src="../../images/imgAdminEdit.gif" alt="Edit" width="14" height="14" border="0" /></a><a href="#" onclick="MM_openBrWindow('../listings/details.php?recordID=<?php echo $row_rsPrograms['id']; ?>','ProgramsDetails','location=yes,scrollbars=yes,width=500')"><img src="../../images/imgAdminView.gif" alt="View" width="14" height="14" /><?php echo substr($row_rsPrograms['ProgramTitle'],0,35)."..."; ?></a> </td>
            ...
            <td>&nbsp;</td>
            <td><a href="#" title="Update User"><img src="../../images/imgSmallUser.gif" alt="reviewers" width="20" height="20" border="0" onclick="MM_openBrWindow('reviewers.php?recordID=<?php echo $row_rsTopics['id']; ?>','Reviewers','scrollbars=yes,width=400')" /></a></td>
...
<?php } while ($row_rsPrograms = mysql_fetch_assoc($rsPrograms)); ?>
        <tr>
          <td colspan="12" nowrap="nowrap" class="tableBottom"><?php echo ($startRow_rsPrograms + 1) ?> to <?php echo min($startRow_rsPrograms + $maxRows_rsPrograms, $totalRows_rsPrograms) ?> of <?php echo $totalRows_rsPrograms ?> </td>
        </tr>
      </table>
