...
mysql_select_db($database_Programming, $Programming);
$query_rsPrograms = sprintf("SELECT id, ProgramTitle, ProgramNumber, `session`, location, FirstName, LastName, EmailAddress, TopicArea, Status FROM callforprograms WHERE `session` = %s OR ProgramTitle LIKE CONCAT('%%', %s, '%%') OR FirstName LIKE CONCAT('%%', %s, '%%') OR LastName LIKE CONCAT('%%', %s, '%%') OR TopicArea = %s AND Status = 'Accepted' ".$_POST['sortby']."", 
...
mysql_select_db($database_Programming, $Programming);
$query_rsPrograms = sprintf("SELECT id, ProgramTitle, ProgramNumber, `session`, location, FirstName, LastName, EmailAddress, TopicArea, Status FROM callforprograms WHERE Status = 'Accepted'");
$query_limit_rsPrograms = sprintf("%s LIMIT %d, %d", $query_rsPrograms, $startRow_rsPrograms, $maxRows_rsPrograms);
$rsPrograms = mysql_query($query_limit_rsPrograms, $Programming) or die(mysql_error());
$row_rsPrograms = mysql_fetch_assoc($rsPrograms);
}
...
<div id="mainContent">
  <div id="mainText">
    <h2><!-- InstanceBeginEditable name="PageTite" --><img src="../../images/PHprograms.jpg" alt="Programs" width="65" height="51" />Program Listings - Accepted <!-- InstanceEndEditable --></h2>
...
?>
	        ...
	        <input name="button" type="button" id="button" onclick="MM_goToURL('parent','accepted.php');return document.MM_returnValue" value="Refresh List" />
	    </form>
	  </li>
	  </ul>
    ...
    <?php if ($totalRows_rsPrograms > 0) { // Show if recordset not empty ?>
      <table border="0" cellpadding="5" cellspacing="0" class="tableborder">
        <tr>
          <td colspan="5" class="tableTop"><?php echo ($startRow_rsPrograms + 1) ?> to <?php echo min($startRow_rsPrograms + $maxRows_rsPrograms, $totalRows_rsPrograms) ?> of <?php echo $totalRows_rsPrograms ?> </td>
          <td class="tableTop">&nbsp;</td>
          <td class="tableTop">&nbsp;</td>
          <td class="tableTop">&nbsp;</td>
          <td colspan="3" class="tableTop"><?php if ($pageNum_rsPrograms > 0) { // Show if not first page ?>
...
?>   class="tableRowColor">
            ...
            <td>&nbsp;</td>
            <td><a href="#" onclick="MM_openBrWindow('details.php?recordID=<?php echo $row_rsPrograms['id']; ?>','ProgramsDetails','scrollbars=yes,width=500')"><?php echo substr($row_rsPrograms['ProgramTitle'],0,35)."..."; ?></a> </td>
            <td>&nbsp;</td>
